use hxo_parser::{Parser, ParserRegistry, MetadataSubParser, TemplateSubParser, ParseState};
use hxo_ir::TemplateNodeIR;
use hxo_types::{HxoValue, Result, Span};
use std::{collections::HashMap, sync::Arc};

struct MockTemplateParser;
impl TemplateSubParser for MockTemplateParser {
    fn parse(&self, _state: &mut ParseState) -> Result<Vec<TemplateNodeIR>> {
        Ok(vec![TemplateNodeIR::Text("Mock".to_string(), Span::default())])
    }
}

struct MockMetadataParser;
impl MetadataSubParser for MockMetadataParser {
...
"#;
    ...
    registry.register_template_parser(Arc::new(MockTemplateParser));
