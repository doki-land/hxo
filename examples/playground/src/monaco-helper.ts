export async function initEditor(container: HTMLElement, initialValue: string, language: string, onChange: (value: string) => void) {
    ...
    if (!monaco.languages.getLanguages().some(lang => lang.id === 'hxo')) {
        ...
        // Define syntax highlighting for HXO
                monaco.languages.setMonarchTokensProvider('hxo', {
                    ...
                    keywords: [
                        'if', 'else', 'for', 'of', 'in', 'return', 'const', 'let', 'var', 'function', 'async', 'await'
                    ],
                    ...
                    tokenizer: {
                        ...
                        string_backtick: [
                            [/[^\\`$]+/, 'string'],
                            [/\\./, 'string.escape'],
                            [/\${/, { token: 'delimiter.bracket', next: '@bracket_counting' }],
                            [/`/, 'string', '@pop']
                        ],
            ...
            provideCompletionItems: () => {
                const suggestions = [
                    ...
                    },
                    {
                        label: 'import-core',
                        kind: monaco.languages.CompletionItemKind.Snippet,
                        insertText: "import { ref, computed, watchEffect } from '@hxo/core';",
                        insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
                    ...
                    },
                    {
                        label: 'computed',
                        kind: monaco.languages.CompletionItemKind.Snippet,
                        insertText: 'const ${1:double} = computed(() => ${2:count} * 2);',
                        insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
